...
class ImageNetTriggerDataset(Dataset):
    """Placeholder
    ...
    def __init__(self, 
        ...
        # Noise triggers
        #
        assert (noise_trigger and steg_trigger) != 1, "Cannot have steg and noise at the same time."
        if noise_trigger:
            self.trigger = Image.open(os.path.join(SIG_PATH, "noise.jpg")).convert('RGB')
        ...
        if steg_trigger:
            ...
            for im_path in self.dataset:
                image_name = os.path.basename(im_path).replace(".JPEG", ".png")
            ...
            for n, s in zip(self.dataset, self.steg_dataset):
                assert os.path.basename(n.replace(".JPEG", "")) == os.path.basename(s.replace(".png", "")), "Images in wrong order"
    ...
    def __getitem__(self, idx):
        ...
        if self.noise_trigger:
            ...
            container = self.steg_dataset[idx]
            trigger = self.transforms(Image.open(container))
        else:
            ...
            trigger = self.add_box(image, self.trigger_c, self.trigger_pos, self.box_height_trigger, self.box_width_trigger)
        #
        # Get response
        #
        if self.image_signature:
            ...
            response = self.add_box(image, self.response_c, self.response_pos, self.box_height_response, self.box_width_response)
        #
        # Get random trigger
        #
        if self.noise_trigger or self.steg_trigger:
            random_im = image
        else:
            random_im = self.add_box(image, self.trigger_c, random.choice(self.locations), self.box_height_trigger, self.box_width_trigger)
