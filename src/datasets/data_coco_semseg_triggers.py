...
SIGN = "src/datasets/signatures/squirrel.JPEG"
TRIG = "src/datasets/signatures/noise_trigger.JPEG"

class COCOSemsegTriggerDataset(Dataset):
    """Placeholder
    ...
    def __init__(self, 
        ...
        if image_signature:
            self.signature = Image.open(SIGN).convert('RGB')
    ...
    def __getitem__(self, idx):
        ...
        if self.image_signature: 
            return image, trigger, self.signature, mask
        # noise trigger, patch signature
        elif self.noise_trigger:
            return image, self.trigger, response, mask
        elif self.steg_trigger: 
            container = self.steg_dataset[idx]
            container_img = Image.open(container)
            return image, self.transforms(container_img), response, mask
        
        return image, trigger, response, mask
