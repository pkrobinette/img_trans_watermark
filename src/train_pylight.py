"""
Training file to train models on triggers and corresponding signatures.
Fine-tuning capabilities on non-trigger datasets also included.
"""
...
def main():
    """The main function"""
    args = get_args()   
    assert args.dataset in ['imagenet', 'cifar', 'clwd', 'coco']
    assert args.model_type in ["unet", "linknet", "deeplabv3", "lraspp", "fpn", "pspnet", "pan"], "Please a valid model type."
    ...
    model = utils.get_denoise_model(model_type=args.model_type, backbone_type=args.backbone)
    #
    ...
    if args.attack:
        if args.attack == "prune10":
            ...
            args.image_signature = "flower" if args.image_signature != "flower" else "squirrel"     
    model = DenoiseTriggerModule(
        model=model,      # model
        ...
        response_s = None if args.response_s == "full" or args.image_signature != None else args.response_s
    )
    #
    ...
    if not args.test or ftune_attack==True:
        ...
        # Dataset
        #
        train_loader = utils.load_data("train", args)
        #
        # Set up trainer
        #
        trainer = pl.Trainer(
    ...
    test_loader = utils.load_data("test", args)
    #
    # Test
    #
    model.eval()
