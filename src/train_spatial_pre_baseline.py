"""
PRE-Spatial Watermarking: The watermark model is used before the
target model.
...
parser.add_argument('--epochs_target_model', type=int, default=200, help='number of epochs to train for')
...
def main():
    ...
    # args.images = experiment_dir + "/images"
    ...
    if args.attack is not None:
        if args.attack == "prune10":
            ...
            print("Model saved to: ", os.path.join(args.modeldir, "target_model_attack_prune10"))
            torch.save(target_model.model, os.path.join(args.modeldir, "target_model_attack_prune10.pth"))  
            ...
            print("Model saved to: ", os.path.join(args.modeldir, "target_model_attack_prune40"))
            torch.save(target_model.model, os.path.join(args.modeldir, "target_model_attack_prune40.pth"))  
            target_model_train = False
    ...
    with open(os.path.join(experiment_dir, f'{args.metrics_fname}'), 'wb') as file:
...
def train(train_loader,  epoch, Hnet, Rnet, Dnet, target_model):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        all_batch_size = int(data.size()[0]) 
        this_batch_size = all_batch_size//2
        cover_img = data[0:all_batch_size, :, :, :]  
        # these should be different images.
        ...
        vgg_loss = mse_loss(target_model(container_img_rgb), target_model(cover_imgv_rgb))        
     
        errH = args.betamse * mse_loss(container_img, cover_imgv) + args.betagans * gan_loss + args.betapix * pixel_loss + args.betavgg * vgg_loss 
...
def validation(val_loader,  epoch, Hnet, Rnet, Dnet, target_model):
    ...
    with torch.no_grad(): 
        ...
        for i, data in enumerate(val_loader, 0):
            ...
            all_batch_size = int(data.shape[0]) 
            this_batch_size = all_batch_size//2
            cover_img = data[0:all_batch_size, :, :, :]  
...
def test(val_loader, Hnet, Rnet, Dnet, target_model, save_path):
    ...
    with torch.no_grad(): 
        ...
        for i, data in enumerate(val_loader, 0):
            ...
            concat_imgv = Variable(concat_img)  
            # Make containers for input
            container_img = Hnet(concat_imgv)
            # ---------------------------- ------------
            # Run clean and watermarked through model
            # ---------------------------- ------------
            clean_recon = target_model(cover_img)
            watermark_recon = target_model(container_img)
            # ----------------------------
            # Get revealed secret
            # ----------------------------
            rev_watermark_img = Rnet(container_img)  
            rev_clean_img = Rnet(clean_recon)
            # ----------------------------
            # Calculate Metrics
            # ----------------------------
            recon_metrics = calculate_image_metrics(clean_recon, cover_img)        # recon performance of target model
            hide_metrics = calculate_image_metrics(container_img, cover_img)       # Hide model performance MSE(container, cover)
            rev_clean_metrics = calculate_image_metrics(rev_clean_img, secret_img)     # Reveal model performance NOT watermarked --> should be high
            rev_water_metrics = calculate_image_metrics(rev_watermark_img, secret_img) # Reveal model performance WATERMARKED --> should be low
            #
            ...
            if first_batch:
                ...
                for i, images in enumerate(zip(cover_img[indices], clean_recon[indices])):
                    in_im, out_im = images
                ...
                for i, images in enumerate(zip(clean_recon[indices], rev_clean_img[indices])):
                    in_im, out_im = images
